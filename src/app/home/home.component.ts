...
export class HomeComponent implements OnInit, AfterViewInit {
  ...
  onWindowResize(event){
    this.headerCanvas.width = (<Element>this.headerCanvasWrapperRef.nativeElement).clientWidth;
    this.headerCanvas.height = (<Element>this.headerCanvasWrapperRef.nativeElement).clientHeight;
  ...
  maxRadius: number = 8;
  minRadius: number = 5;
  minDistanceBetweenParticles: number = 20;
  ...
  doesParticlesAreNearby(particle1: HomeParticle, particle2: HomeParticle): boolean {
    let d =  Math.sqrt( Math.pow(particle1.x - particle2.x, 2) + Math.pow(particle1.y - particle2.y, 2));
